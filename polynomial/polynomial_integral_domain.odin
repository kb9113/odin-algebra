...
// makes an integral domain structure for a polynomial given the coefficents integral domain structure
make_polynomial_integral_domain :: proc($T : typeid, coefficent_structure : $ST) ->
    integral_domain.IntegralDomain(Polynomial(T, ST)) where intrinsics.type_is_subtype_of(ST, integral_domain.IntegralDomain(T))
{
    ...
    return integral_domain.IntegralDomain(Polynomial(T, ST)){
        make_polynomial_commutative_ring(T, coefficent_structure),
        l_cancel
    }
}
...
// sets ans equal to p canceld by q ie if p = a * q then cancel(p, q) == a
// this procedure panics if q does not divide p ie if p is not a muliple of q
cancel :: proc(ans : ^Polynomial($T, $ST), p : Polynomial(T, ST), q : Polynomial(T, ST))
{
    ...
    if rawptr(raw_data(p.coefficents)) == rawptr(raw_data(q.coefficents))
    {
        // if p == q clearly p / q == 1
        resize_or_init_polynomial(ans, p.algebraic_structure, 0)
        p.algebraic_structure.set(&ans.coefficents[0], p.algebraic_structure.mul_identity)
        return
    }

    divisor : Polynomial(T, ST)
    ans_eq_q := rawptr(raw_data(ans.coefficents)) == rawptr(raw_data(q.coefficents))
    if ans_eq_q
    {
    ...
    for i in 0..<(deg_p - deg_q + 1)
    {
        p.algebraic_structure.cancel(
            &ans.coefficents[deg_p - deg_q - i],
            curr_remainder.coefficents[deg_p - i],
            divisor.coefficents[deg_q]
        )
        ...
        for j in 1..=deg_q
        {
            p.algebraic_structure.mul(
                &curr_remainder.coefficents[deg_p - i],
                ans.coefficents[deg_p - deg_q - i],
                divisor.coefficents[deg_q - j]
            )
            p.algebraic_structure.sub(
                &curr_remainder.coefficents[deg_p - j - i],
                curr_remainder.coefficents[deg_p - j - i],
                curr_remainder.coefficents[deg_p - i]
            )
        }
